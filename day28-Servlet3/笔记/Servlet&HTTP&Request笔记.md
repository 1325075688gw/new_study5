...
## Servlet：
	...
	7. Servlet相关配置
		1. urlpartten:Servlet访问路径
			...
			2. 路径定义规则：
				1. /xxx：路径匹配                   /*(优先级非常低)
				2. /xxx/xxx:多层路径，目录结构       /xxx/*  
				3. *.do：扩展名匹配                 /*.do(不合法)  扩展名匹配不能加/  应该这么写 "*.do"
		...
		1. 请求行
			...
			* 请求方式：
				* HTTP协议有7中请求方式，常用的有2种
					* GET：
						1. 请求参数在请求行中，在url后。
						    http:localhost/test?username=zhangsan
						    
						    POST /login.html?username=zhangsan	HTTP/1.1
						2. 请求的url长 度有限制的
						3. 不太安全
					* POST：
						1. 请求参数在请求体中
			...
			* 常见的请求头：
				...
				2. Referer：http://localhost/login.html
					* 告诉服务器，我(当前请求)从哪里来？
						* 作用：
							1. 防盗链：
							2. 统计工作：[给百度,腾讯,微博投放广告,看谁到达我这的链接数最多,下次就投递谁家广告]
		3. 请求空行
			空行，就是用于分割POST请求的请求头，和请求体的。
		4. 请求体(正文)：
			* 封装POST请求消息的请求参数的 [GET没有请求体,GET请求的请求参数会放在URL后面,以问号?开始,等号=链接,多组参数,用&连接]
